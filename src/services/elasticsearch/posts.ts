...
import { countTotalPages, esHitsMap, esTotalHits, getQueryStringPage } from './helpers';
...
export async function esGetLastPostsPaginate(queryStrings: QueryStringParsed, size = 10) {
...
export async function esQueryCategoryPosts(categorySlug: string, size = 3, from = 0) {
  const query = {
    from,
    size,
    sort: [{ createdAt: { order: 'desc' } }],
...
export async function esGetCategoryPostsPaginate(categorySlug: string, queryStrings: QueryStringParsed, size = 10) {
  const page = getQueryStringPage(queryStrings);
  const from = size * page - size;
